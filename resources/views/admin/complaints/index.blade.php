...
@section('content')
    ...
    <div class="mb-5 dashboard-header">
        <div class="d-flex justify-content-between align-items-start mb-1">
            <div>
                <h2 class="text-white mb-0" style="font-size: 2.25rem; font-weight: 700; line-height: 1.2; margin-top: 0 !important;">Complaints Management</h2>
            </div>
        ...
        <p class="text-light mb-0" style="margin-top: 0.5rem; padding-left: 0.5rem; font-size: 1.1rem; opacity: 0.85;">
        ...
        <div class="card-body">
            <form id="complaintsFiltersForm" method="GET" action="{{ route('admin.complaints.index') }}">
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-auto">
                        <label class="form-label small mb-1"
                            style="font-size: 0.8rem; color: #000000 !important; font-weight: 500;">House No.</label>
                        <input type="text" class="form-control" id="houseNoInput" name="house_no" placeholder="House No..."
                            value="{{ request('house_no') }}" oninput="handleComplaintsSearchInput()"
                            style="font-size: 0.9rem; width: 140px;">
                    </div>
                    <div class="col-auto">
                        <label class="form-label small mb-1"
                            style="font-size: 0.8rem; color: #000000 !important; font-weight: 500;">Priority</label>
                        ...
                        <select class="form-select" name="assigned_employee_id" onchange="submitComplaintsFilters()"
                            ...
                            @foreach($employees as $employee)
                                <option value="{{ $employee->id }}" {{ request('assigned_employee_id') == $employee->id ? 'selected' : '' }}>
                                    {{ $employee->name }}@if($employee->designation)
                                    ({{ $employee->designation->name ?? $employee->designation }})@endif
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-auto">
                        <label class="form-label small mb-1"
                            style="font-size: 0.8rem; color: #000000 !important; font-weight: 500;">From</label>
                        <input type="date" class="form-control" name="date_from" value="{{ request('date_from') }}"
                            onchange="submitComplaintsFilters()" style="font-size: 0.9rem; width: 150px;">
                    </div>
                    <div class="col-auto">
                        <label class="form-label small mb-1"
                            style="font-size: 0.8rem; color: #000000 !important; font-weight: 500;">To</label>
                    ...
                    <div class="col-auto">
                        ...
                        <select class="form-select" name="status" onchange="submitComplaintsFilters()"
                            ...
                            <option value="product_na" {{ request('status') == 'product_na' ? 'selected' : '' }}>Product N/A
                            </option>
                    ...
                    <div class="col-auto">
                        ...
                        <button type="button" class="btn btn-outline-secondary btn-sm" onclick="resetComplaintsFilters()"
    ...
    <div class="card-glass" style="padding: 0 !important; border-radius: 12px; overflow: hidden;">
        ...
        <div class="card-body" style="padding: 0 !important; margin: 0 !important;">
            <div class="table-responsive-xl"
                style="margin: 0 !important; padding: 0 !important; border-radius: 0 0 12px 12px; overflow: hidden;">
                <table class="table table-dark table-sm table-compact"
                    style="margin: 0 !important; border-left: none !important; border-radius: 0 0 12px 12px;">
                    <thead>
                        <tr>
                    ...
                    <tbody id="complaintsTableBody">
                        @forelse($complaints as $complaint)
                            <tr>
                                ...
                                <td style="white-space: nowrap;">
                                    {{ $complaint->created_at ? $complaint->created_at->timezone('Asia/Karachi')->format('M d, Y H:i:s') : '' }}
                                </td>
                                <td style="white-space: nowrap; text-align: {{ $complaint->closed_at ? 'left' : 'center' }};">
                                    {{ $complaint->closed_at ? $complaint->closed_at->timezone('Asia/Karachi')->format('M d, Y H:i:s') : '-' }}
                                </td>
                                ...
                                <td>
                                    <div class="btn-group" role="group">
                                        <button onclick="viewComplaint({{ $complaint->id }})"
                                            class="btn btn-outline-success btn-sm" title="View Details"
                                        ...
                                        @if($complaint->status === 'resolved' || $complaint->status === 'closed')
                                            <button class="btn btn-outline-secondary btn-sm"
                                                ...
                                                style="padding: 3px 8px; cursor: not-allowed; opacity: 1 !important;" disabled>
            ...
            <!-- TOTAL RECORDS -->
            <div id="complaintsTableFooter" class="text-center py-2 mt-2"
...
@push('styles')
    <style>
        ...
        /* Match approvals index table text size */
